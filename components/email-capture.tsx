...
export function EmailCapture({ context }: EmailCaptureProps) {
  ...
  if (success) {
    return (
      <div className="retro-panel px-5 py-5 text-center">
        <p className="retro-kicker">Transmission received</p>
        <p className="mt-3 text-sm font-medium text-emerald-100">
          You&apos;re in. We&apos;ll beam over job search tips, resume strategies, and career
          intel. No spam. Easy unsubscribe.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="retro-panel rounded-[2rem] p-6 sm:p-8">
      <p className="retro-kicker">Bonus level</p>
      <h3 className="mt-2 text-2xl font-black uppercase text-white">
        Get weekly job search signals in your inbox
      </h3>
      <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-300">
        Resume strategies, interview prep, salary negotiation tips, and career insights.
        Fresh signals only. No junk mail side quests.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-5 flex flex-col gap-3 sm:flex-row">
        <input
          ...
          required
          className="min-w-0 flex-1 rounded-full border border-cyan-400/20 bg-slate-950/80 px-5 py-3 text-sm text-slate-100 outline-none transition focus:border-fuchsia-300 focus:shadow-[0_0_0_3px_rgba(232,121,249,0.12)]"
        />
        <button
          type="submit"
